function params = learnParameters(pathNewTrainingFolder, cues, dir_root)
...
elseif params.primary_type == params.TYPE_VIDEO
    struct = load([params.trainingVideos 'structGT.mat']);
end
...
for idx = 1:length(structGT)
    ...
    elseif structGT(idx).type == params.TYPE_VIDEO
        V = VideoReader(fullfile(params.trainingVideos, structGT(idx).video_file));
