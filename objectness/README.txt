...
2. build a struct analog to structGT (see Training/Images/structGT.mat and Training/Videos/structGT.mat) and save it in NewTrainingFolder under the name 'structGT.mat'. This struct should have the following fields:
    i. field 'name' : 1 if the current example is an image, 2 if it is a video
    ii-a. the name of the image (field 'img') in NewTrainingFolder
    ii-b. the name of the video (field 'video_file') and the selected frame (field 'frame')
    iii. ground-truth bounding-boxes for each object in the image (field 'boxes').
